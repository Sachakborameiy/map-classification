...
export default function MapClassification() {
  const [provincesCity, setProvinces] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchProvinces = async () => {
      try {
        const response = await axios.get("http://localhost:8100/province/", {
          headers: {
                ...
                <div className="w-[75%] relative">
                  <div>
                    ...
                    >
                      ...
                      {provincesCity.map((province_city, index) => (
                        <option key={index} value={province_city}>
                          {province_city}
                        </option>
                      ))}
                    </select>
